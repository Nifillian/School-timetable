...
<h1 class="head">Добавление замены классу <?php echo $class_name ?> класса</h1>
...
<form method="post">
    ...
    <input type="date" id="date" name="date" required
        min="<?php echo date('Y-m-d', strtotime('last Sunday + 1 day')); ?>"
        max="<?php echo date('Y-m-d', strtotime('next Monday + 6 day')); ?>">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if the selected cabinet and teacher is free on this day and time
    $sql_check_cabinet = "SELECT * FROM substitutions WHERE cabinet_id = $cabinet_id AND date_of_day = '$date_of_day' AND number = $number";
    $sql_check_teacher = "SELECT * FROM substitutions WHERE teacher_id = $teacher_id AND date_of_day = '$date_of_day' AND number = $number";
    $sql_check_lesson = "SELECT * FROM substitutions WHERE date_of_day = '$date_of_day' AND class_id = $class_id AND number = $number";
    $result_check_cabinet = mysqli_query($connection, $sql_check_cabinet);
    $result_check_teacher = mysqli_query($connection, $sql_check_teacher);
    ...
    if (mysqli_num_rows($result_check_cabinet) == 0 && mysqli_num_rows($result_check_lesson) == 0) {
        if (mysqli_num_rows($result_check_teacher) == 0) {
            ...
            $sql = "INSERT INTO substitutions (number, class_id, subject_id, cabinet_id, teacher_id, date_of_day) VALUES ('$number', '$class_id', '$subject_id', '$cabinet_id','$teacher_id', '$date_of_day')";
            ...
            echo "<br><p>Преподаватель занят на эту дату и время.</p>";
        }
