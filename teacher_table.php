<?php
        ...
        function getTeacherData($connection, $teacher_id, $startOfWeek, $endOfWeek) {
            $sql = "SELECT lesson.number, subjects.name AS subjects, classes.name AS class, cabinets.name AS cabinets, lesson.date_of_day AS date
                    FROM lesson
        ...
        function getSubstitutionTeacherData($connection, $teacher_id, $startOfWeek, $endOfWeek) {
          $sql = "SELECT substitutions.number, subjects.name AS subjects, classes.name AS class, cabinets.name AS cabinets, substitutions.date_of_day AS date
                  FROM substitutions
                  ...
                  WHERE substitutions.teacher_id = '$teacher_id'
      ...
      $endOfWeek = '2023-06-03';
        ...
        $data = getTeacherData($connection, $teacher_id, $startOfWeek, $endOfWeek);
...
<html>
<head>
  <title>Расписание для <?php echo $teacher_name ?></title>
...
<header>
  ...
  <h1 class="head">Расписание <?php echo $teacher_name ?></h1>
...
<div class=container>
  ...
  foreach ($weekDays as $day) {
    ...
    $data = getTeacherData($connection, $teacher_id, $day, $day);
    $fullScheduleData = getTeacherData($connection, $teacher_id, $day, $day);
    $substitutionsData = getSubstitutionTeacherData($connection, $teacher_id, $currentDateSub, $currentDateSub);
    // Проверка, есть ли записи для текущего дня
    if ($data->num_rows > 0 || $substitutionsData->num_rows > 0) {
      echo "<h2 class=table>" . $dayOfWeek . "</h2>";
      echo "<table>";
      echo "<thead><tr><th class=numb> № </th><th> Время </th><th> Урок </th><th> Класс </th><th> Кабинет </th></tr></thead>";
      echo "<tbody>";
